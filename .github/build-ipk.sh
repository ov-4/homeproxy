...
po2lmo "$PKG_DIR/po/zh_Hans/ov4proxy.po" "$TEMP_PKG_DIR/usr/lib/lua/luci/i18n/ov4proxy.zh-cn.lmo"

if [ "$PKG_MGR" == "apk" ]; then
	find "$TEMP_PKG_DIR" -type f,l -printf '/%P\n' | sort > "$TEMP_PKG_DIR/lib/apk/packages/$PKG_NAME.list"
	echo "/etc/config/ov4proxy" >> "$TEMP_PKG_DIR/lib/apk/packages/$PKG_NAME.conffiles"
...
default_prerm' > "$TEMP_DIR/pre-deinstall"
	...
	apk mkpkg \
		...
		--info "arch:all" \
		--info "origin:https://github.com/ov-4/ov4proxy" \
		--info "url:" \
	...
	cat > "$TEMP_PKG_DIR/CONTROL/control" <<-EOF
		...
		Source: https://github.com/ov-4/ov4proxy
		SourceName: $PKG_NAME
		Section: luci
	...
	echo -e "/etc/config/ov4proxy" > "$TEMP_PKG_DIR/CONTROL/conffiles"
