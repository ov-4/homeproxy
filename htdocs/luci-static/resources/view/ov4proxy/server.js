...
function getServiceStatus() {
	return L.resolveDefault(callServiceList('ov4proxy'), {}).then((res) => {
		let isRunning = false;
		try {
			isRunning = res['ov4proxy']['instances']['sing-box-s']['running'];
		} catch (e) { }
		return isRunning;
	});
}
...
function renderStatus(isRunning, version) {
	...
	let renderHTML;
	if (isRunning)
		renderHTML = spanTemp.format('green', _('ov4proxy Server'), version, _('RUNNING'));
	else
		renderHTML = spanTemp.format('red', _('ov4proxy Server'), version, _('NOT RUNNING'));
...
return view.extend({
	...
	render(data) {
		...
		m = new form.Map('ov4proxy', _('ov4proxy Server'),
		...
		s = m.section(form.NamedSection, 'server', 'ov4proxy', _('Global settings'));
		...
		*/
		...
		o.onchange = function(ev, section_id, value) {
			let desc = this.map.findElement('id', 'cbid.ov4proxy.%s.transport'.format(section_id)).nextElementSibling;
			if (value === 'http')
			...
			let tls_element = this.map.findElement('id', 'cbid.ov4proxy.%s.tls'.format(section_id)).firstElementChild;
			if ((value === 'http' && tls_element.checked) || (value === 'grpc' && !features.with_grpc))
				this.map.findElement('id', 'cbid.ov4proxy.%s.http_idle_timeout'.format(section_id)).nextElementSibling.innerHTML =
		...
		o.validate = function(section_id, value) {
			if (section_id) {
				...
				let tls = this.map.findElement('id', 'cbid.ov4proxy.%s.tls'.format(section_id)).firstElementChild;
		...
		o.value('/etc/ov4proxy/certs/server_publickey.pem');
		...
		o.depends({'tls': '1', 'tls_cert_path': '/etc/ov4proxy/certs/server_publickey.pem'});
		...
		o.value('/etc/ov4proxy/certs/server_privatekey.pem');
		o.depends({'tls': '1', 'tls_acme': '0', 'tls_reality': '0'});
		o.depends({'tls': '1', 'tls_acme': '0', 'tls_reality': null});
		o.depends({'tls': '1', 'tls_acme': null, 'tls_reality': '0'});
		o.depends({'tls': '1', 'tls_acme': null, 'tls_reality': null});
		...
		o.inputstyle = 'action';
		o.inputtitle = _('Upload...');
		o.depends({'tls': '1', 'tls_key_path': '/etc/ov4proxy/certs/server_privatekey.pem'});
