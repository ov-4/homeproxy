...
function renderNodeSettings(section, data, features, main_node, routing_mode) {
	...
	o.onchange = function(ev, section_id, value) {
		...
		if ((value === 'http' && tls.checked) || (value === 'grpc' && !features.with_grpc)) {
			...
			this.map.findElement('id', 'cbid.ov4proxy.%s.http_ping_timeout'.format(section_id)).nextElementSibling.innerHTML =
	...
	o.value('/etc/ov4proxy/certs/client_ca.pem');
	...
	o.depends({'tls_self_sign': '1', 'tls_cert_path': '/etc/ov4proxy/certs/client_ca.pem'});
	...
	o.value('/etc/ov4proxy/certs/client_ech_conf.pem');
	o.depends('tls_ech', '1');
	o.modalonly = true;
	...
	o.inputstyle = 'action';
	o.inputtitle = _('Upload...');
	o.depends({'tls_ech': '1', 'tls_ech_config_path': '/etc/ov4proxy/certs/client_ech_conf.pem'});
	...
	if (features.with_utls) {
		...
		o.validate = function(section_id, value) {
			if (section_id) {
				let tls_reality = this.map.findElement('id', 'cbid.ov4proxy.%s.tls_reality'.format(section_id)).firstElementChild;
...
return view.extend({
	...
	render(data) {
		...
		m = new form.Map('ov4proxy', _('Edit nodes'));

		s = m.section(form.NamedSection, 'subscription', 'ov4proxy');
		...
		o.placeholder = 'Wget/1.21 (ov4proxy, like v2rayN)';
		...
		o.onclick = function() {
			return fs.exec_direct('/etc/ov4proxy/scripts/update_subscriptions.uc').then((res) => {
